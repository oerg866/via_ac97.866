...
// #define _DBG(x) vfm_tsrOutPortB(0x80, (unsigned char) x)
#define _DBG(x)
...
// #pragma code_seg("ResidentCode", "CODE")


/*  ASM optimizations for 32 bit math 
    Code in VFM_32BT.ASM */

/* Signed Long division out = a / b */
extern void __ldiv(int32_t *_out, int32_t _a, int32_t b);
/* Unsigned Long Multiplication out = a * b */
extern void __ulmul(uint32_t *_out, uint32_t _a, uint32_t b);
/* Signed Long Multiplication out = a * b */
extern void __lmul(int32_t *_out, int32_t _a, int32_t b);
/* Signed Long Shift Left out = a << b */
extern void __lshl(int32_t *_out, int32_t _a, int32_t b);
/* Signed Long Shift Right out = a >> b */
extern void __lshr(int32_t *_out, int32_t _a, int32_t b);
/* Signed 64 bit Long Shift Right out >>= a */
extern void __llshr(_uint64 *_out, uint32_t a);
extern void __i64add32(_uint64 *_out, int32_t _a);
...
static int16_t OPL3_EnvelopeCalcExp(uint32_t level)
{
    ...
    return (exprom[level & 0xffu] << 1) >> (((uint16_t) level) >> 8);
}
...
static void OPL3_PhaseGenerate(opl3_slot *slot)
{
    ...
    {
        uint32_t tmp_phase;
        __lshr(&tmp_phase, slot->pg_phase, 9);
        phase = (uint16_t) tmp_phase;
    }
#endif
    if (slot->pg_reset)
    {
    ...
    {
        uint32_t tmp_mul;
        __ulmul(&tmp_mul, basefreq, (u32) mt[slot->reg_mult]);
        __lshr(&tmp_mul, tmp_mul, 1);
        slot->pg_phase += tmp_mul;
    }
#endif
    /* Rhythm mode */
    noise = chip->noise;
    ...
    chip->noise = (noise >> 1) | ((uint32_t) n_bit << 22);
#else
    {
        uint32_t noise_shifted;
        __lshr(&noise_shifted, noise, 14);
        n_bit = (noise_shifted ^ noise) & 0x01;
    }
    {
        uint32_t n_bit_shifted;
        __lshr(&chip->noise, noise, 1 );
        __lshl(&n_bit_shifted, (uint32_t) n_bit, 22);
        chip->noise |= n_bit_shifted;        
    }
#endif
}
...
void OPL3_GenerateResampled(opl3_chip *chip, int16_t *buf)
{
    int16_t samples[4];
    _DBG(0xf2);
    OPL3_Generate2ChResampled(chip, samples);
    buf[0] = samples[0];
    buf[1] = samples[1];
}

static void OPL3_Memset(void *ptr, uint8_t value, uint16_t size) {
...
void OPL3_Reset(opl3_chip *chip, uint32_t samplerate)
{
    ...
    OPL3_Memset(chip, 0, sizeof(opl3_chip));
